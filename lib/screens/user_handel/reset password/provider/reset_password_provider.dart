...
import 'package:amit_project/screens/user_handel/reset%20password/provider/reset_password_state.dart';
...
class ResetPasswordProvider extends ChangeNotifier {
  ...
  sendEmail(BuildContext context) {
    Navigator.of(context).pushNamed(App_Routes.emailSent);
  }

  openEmailApp(BuildContext context) {
    Navigator.of(context).pushNamed(App_Routes.chooseNewPass);
  }

  void showNewPassword() {
    state.hideNewPass == true
        ? state.hideNewPass = false
        : state.hideNewPass = true;
    notifyListeners();
  }

  void showConfirmNewPassword() {
    state.hideConfirmNewPass == true
        ? state.hideConfirmNewPass = false
        : state.hideConfirmNewPass = true;
    notifyListeners();
  }
  ...
  successScreen(BuildContext context) {
    Navigator.of(context).pushNamed(App_Routes.resetSuccessScreen);
  }
}
